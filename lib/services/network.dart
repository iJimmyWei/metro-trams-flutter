...
class NetworkHelper {
  Future<List<String>> lookupStationNames(String searchInput) async {
    ...
    var url =
        'https://api.tfgm.com/odata/Metrolinks?\$filter=contains(stationLocation, \'$searchInput\')';
    var response = await http.get(url,
        headers: {'Ocp-Apim-Subscription-Key': DotEnv().env['API_KEY']});
  ...
  Future<List<Station>> getStationData(String searchInput) async {
    ...
    if (searchInput.contains("'")) {
      ...
      url =
          'https://api.tfgm.com/odata/Metrolinks?\$filter=contains(stationLocation, \'$searchQuery\')';
    } else {
      url =
          'https://api.tfgm.com/odata/Metrolinks?\$filter=(stationLocation eq \'$searchInput\')';
    }
